...
st.markdown(
    """The page below offers insight into several key metrics from the data, including PMPM,
...
filtered_pmpm_data = pmpm_data.loc[
    (pmpm_data["year_month"] >= start_date) & (pmpm_data["year_month"] <= end_date), :
]
...
st.divider()
st.markdown(
    """Inpatient and Outpatient spend are the largest drivers of PMPM during this time period"""
)
plost.bar_chart(
    data=grouped_pmpm,
    bar="Metric",
...
st.markdown(
    """Pharmacy Spend appears largely steady between 2016 and 2018, averaging between `$100k-$200k` a month.
...
msk = (cond_data["diagnosis_year_month"] >= str(start_date)) & (
    cond_data["diagnosis_year_month"] <= str(end_date)
)
